...
const { Server } = require("socket.io"); // Importação correta
...
const io = new Server(server, {
    cors: {
        origin: "*", // Aceita tudo (Celular, Emulador, PC)
        methods: ["GET", "POST"],
        credentials: false
    },
    allowEIO3: true // Compatibilidade extra para redes móveis
});
...
const ITEM_RESPAWN_TIME = 60 * 60 * 1000;
const NINJA_DMG = 15;
...
// --- CARREGAMENTO SEGURO DO MAPA ---
// Usamos try/catch para garantir que o servidor NÃO CAIA se o arquivo faltar
try {
    ...
    if (fs.existsSync(mapPath)) {
        ...
        console.log(`[SERVER] Mapa carregado: ${mapPlatforms.length} plataformas.`);
    } else {
        console.log("[SERVER] Mapa não encontrado, usando padrão.");
        mapPlatforms.push({ x: 0, y: 600, w: 2000, h: 50 });
    }
} catch (error) {
    console.error("[SERVER] Erro ao ler mapa:", error.message);
    mapPlatforms.push({ x: 0, y: 600, w: 2000, h: 50 });
}
...
function restartMatch() {
    ...
    Object.keys(players).forEach(id => {
        ...
        players[id].isDead = false; players[id].x = 200 + Math.random() * 400; players[id].y = 100;
    });
